...
// Lazy initialization for Firestore to avoid calling firestore() before initializeApp()
...
function checkRateLimit(phone: string): { allowed: boolean; error?: string } {
  const now = Date.now();
...
/**
 * Standardizes a Korean phone number to E.164 format.
 * Expects '01012345678' and returns '+821012345678'
 */
...
export const sendVerificationCode = functions.region('asia-northeast3').https.onCall(async (data: any, context: any) => {
  ...
  try {
    ...
    // const loginUrl = `${process.env.FRONTEND_URL || 'https://sookmyung-97032.web.app'}/phone-auth?phone=${encodeURIComponent(normalizedPhone)}`;
    ...
    try {
      ...
      // Don't throw here - we still want to store the verification code
    }

    // Store verification code in Firestore regardless of AlimTalk status
    ...
    // If AlimTalk failed, provide a helpful message with the code in development mode
    if (!alimtalkSuccess) {
      ...
      return {
        ...
        code: isDevelopment ? code : undefined, // Only include code in development
...
export const verifyCode = functions.region('asia-northeast3').https.onCall(async (data: any, context: any) => {
  ...
  // TEST USER: 010-1234-1234 with code "123456"
...
// Removed graduation_year field from registration and login - Feb 2026
export const registerWithPhone = functions.region('asia-northeast3').https.onCall(async (data: any, context: any) => {
  ...
  try {
    ...
    // Standardize phone for Firebase Auth
    const authPhone = toE164(phone);

    try {
      // Attempt to find existing user with this phone number in Auth
      ...
      // If it still fails with phone conflict, handle specifically
...
export const loginWithPhone = functions.region('asia-northeast3').https.onCall(async (data: any, context: any) => {
...
export const requestPasswordReset = functions.region('asia-northeast3').https.onCall(async (data: any, context: any) => {
...
export const fetchUserByPhone = functions.region('asia-northeast3').https.onCall(async (data: any, context: any) => {
  const { phone } = data;
