...
const ConfigManagement: React.FC<ConfigManagementProps> = ({
  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-5xl">
      ...
      {/* Default Amount */}
      <div className="bg-white rounded-4xl border border-gray-100 shadow-soft p-8">
        <div className="flex items-center gap-3 mb-6">
          <ShieldCheck className="w-6 h-6 text-sookmyung-blue-800" />
          <div>
            <h3 className="text-xl font-bold text-gray-900">기본 참가비 (폴백)</h3>
            <p className="text-sm text-gray-500 mt-0.5">활성 구간이 없을 때 자동으로 적용되는 기본 금액입니다. 항상 최신 정가로 유지하세요.</p>
          </div>
        </div>
        <div className="relative max-w-xs">
          <input
            type="number"
            value={defaultAmount}
            onChange={(e) => onDefaultAmountChange(parseInt(e.target.value) || 0)}
            className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-4 py-3 text-lg font-bold text-gray-900 focus:ring-2 focus:ring-sookmyung-blue-600 outline-none transition-all text-right pr-12"
            placeholder="280000"
            min={0}
          />
        ...
        <p className="text-xs text-gray-400 mt-3">
          현재 설정: <strong className="text-gray-700">{defaultAmount.toLocaleString()}원</strong>
        </p>
      </div>
                  ...
                  <div className="md:col-span-4">
                    ...
                    <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-2xl p-1 shadow-inner">
                      <input
                        ...
                        className="flex-1 border-none rounded-xl px-3 py-2 text-xs focus:ring-0 outline-none hover:bg-gray-50 transition-colors"
                      />
